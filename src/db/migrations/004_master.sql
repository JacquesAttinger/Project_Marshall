...
-- agent also keeps the issue identifier (names every per-issue file), the plan path, the planner
-- model, the PR URL, the fresh-restart count, and the merged PR that queued a rebase on the row.
...
ALTER TABLE claims ADD COLUMN identifier     TEXT;
