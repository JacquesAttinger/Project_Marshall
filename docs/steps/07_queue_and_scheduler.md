...
A loop that never starts more than the caps allow, never starts two agents on overlapping code, and never leaves an issue claimed with no agent behind it.
...
- Overlap check `canStart(issue, activeClaims)`: first version compares the issue's likely paths with each active claim's "Likely touched files" from its plan. Likely paths for the new issue come from a Haiku guess over the issue text (see step 04, open question 2). If unsure, allow.
...
- Overlap by anything smarter than path lists. Iteration 2.

## Reuse (search first)

- `~/.claude/hooks/session-start-linear-suggest.sh` — priority-sorted query.
- `~/code/hemut3/hemut-ai-harness/skills/shared/surface-check/` — collision check on file surfaces; borrow the comparison logic.
...
2. When the overlap check is unsure, allow or wait? Waiting is safer but can starve a big issue behind two small ones.
